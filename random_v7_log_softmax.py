...
class DirectorChain(Chain):
    ...
    def fwd(self,x):
         ...
         h0=F.log_softmax(self.l0(h_input))
         h1=F.log_softmax(self.l1(h0))
         h2=F.log_softmax(self.l2(h1))
         h3=F.log_softmax(self.l3(h2))
         h_output = self.l_output(h3)
         return h_output


class random_v7_log_softmax(director):
    description="活性化関数がlog_softmax。"
    model=None
    x_len=0
    y_len=0
